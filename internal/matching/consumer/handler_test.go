...
func TestHandler_readOnlySkipsNewOrder(t *testing.T) {
	dir := t.TempDir()
	eng, err := recovery.Open(testRecoveryConfig(dir))
	if err != nil {
		t.Fatal(err)
	...
	eng.SetSymbolReadOnly("BTC-USDT", "test")
	pub := publisher.NewMemory()
	h := &consumer.Handler{Engine: eng, Publisher: pub, Partition: 0}

	cmd := recovery.NewOrderFromEngine(engine.Order{
		OrderID: 3, Symbol: "BTC-USDT", Side: engine.SideSell, Type: engine.OrderTypeLimit,
	...
	payload, _ := proto.Marshal(env)
	if err := h.Process(context.Background(), kafka.Message{Partition: 0, Offset: 3, Value: payload}); err != nil {
		t.Fatal(err)
	}
	if len(pub.MatchEvents()) != 0 {
		t.Fatal("read-only symbol should not publish")
	}
	if eng.LastSeq() != 0 {
		t.Fatalf("read-only should not write WAL, last_seq=%d", eng.LastSeq())
	}
}
