...
// Writer 将 WAL 记录顺序追加到本地磁盘；每次 Append 后 durable sync。
...
func OpenFileWriter(dir string, cfg FileWriterConfig) (*FileWriter, error) {
	...
	w := &FileWriter{dir: dir, cfg: cfg, writeBufferSize: bufSize}
...
// Append 编码并写入一条记录，刷缓冲并 durable sync。
...
// AppendNext 分配递增 seq 并追加（seq = lastSeq+1），减少上层 LastSeq 再 Append 的重复逻辑。
